...
module "oci-fk-wordpress2" {
  ...
  numberOfNodes             = 1
  availability_domain       = var.mds_availability_domain2 == "" ? lookup(data.oci_identity_availability_domains.ADs2.availability_domains[0], "name") : var.mds_availability_domain2
  compartment_ocid          = var.mds_compartment_ocid
  image_id                  = lookup(data.oci_core_images.OSImage2.images[0], "id")
  shape                     = var.wordpress_compute_shape
  flex_shape_ocpus          = var.wordpress_compute_flex_shape_ocpus
  flex_shape_memory         = var.wordpress_compute_flex_shape_memory
  ssh_authorized_keys       = tls_private_key.public_private_key_pair.public_key_openssh
