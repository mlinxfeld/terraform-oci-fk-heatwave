resource "oci_mysql_mysql_backup" "FoggyKitchenMDSManualBackup" {
    count             = var.mds_manual_backup_enabled && !(var.mds_shape == "MySQL.Free") && !(var.mds_channel_enabled) && !(var.mds_replica_enabled) ? 1 : 0
...
resource "oci_mysql_mysql_backup" "FoggyKitchenMDSCrossRegionBackup" {
    count             = var.mds_cross_region_manual_backup_enabled  && !(var.mds_channel_enabled) && !(var.mds_replica_enabled) ? 1 : 0
    source_details {
